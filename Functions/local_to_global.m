...
x_max = xmax_fix+(xmax_mov/2);
y_max = ymax_fix+(ymax_mov/2);
% Global coordinates
[global_fix] = resize_map(local_fix, x_max, y_max);
[global_mov] = resize_map(local_mov, x_max, y_max);
end

function [resized_map] = resize_map(map, x_max, y_max)
% Get the resolution of the map
res = map.Resolution;
% Create an empty map with the same resolution as the input map
resized_map = occupancyMap(x_max, y_max, res);
